...
cp .env.example .env   # depois edite o .env e preencha os valores (veja "Google Drive" abaixo)
...
- **Como funciona:** a cada edição de conteúdo/título, o estado *anterior* à mudança é salvo como uma versão — mas só quando isso é um checkpoint que faz sentido: se a pessoa **pausou por pelo menos 60 segundos** desde a última edição (sinal real de que uma sessão de edição terminou), ou se já passou 10 minutos desde a última versão salva mesmo sem pausa (rede de segurança para sessões contínuas muito longas). *(Nota: a primeira versão desta funcionalidade usava um intervalo fixo de 5 minutos desde a última versão, o que fazia sessões de teste curtas nunca gerarem nada além do estado inicial em branco — corrigido para o modelo por pausa acima.)*
